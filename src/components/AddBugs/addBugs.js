...
const AddBugs = (props) => {
  ...
  const chooseApp = apps ? 
    apps.apps.map(item => {
    return <option value ={item.app_name}>{item.app_name}</option>
    })
  : null;
 ...
 return (
  <div>
    ...
    <form className='newBug'>
      <select
      onChange={(ev) => {setApp(ev.currentTarget.value)}}
      >
        {chooseApp}
      </select>
      <label htmlFor='bugName'>What is the bug?</label>
      <input id='bugName'
      onChange={(ev) => {setBugName(ev.currentTarget.value)}}
      ></input>
      <label htmlFor='bugDescription'>Please describe the bug in detail</label>
